...
test.describe("Ensure I can change serial to real @hipanel-module-stock", () => {
  ...
  test.beforeEach(async ({ page }) => {
    indexPage = new Index(page);
    await page.goto("/stock/part/index?PartSearch[partno]=SC815TQ-600WB&PartSearch[dst_name_in]=TEST-DS-02");
    await expect(page).toHaveTitle("Parts");
    await indexPage.hasRowsOnTable(1);
  });

  test("Set serial as part ID @manager", async ({ page }) => {
    await indexPage.chooseNumberRowOnTable(1);
    await indexPage.clickDropdownBulkButton("Bulk actions", "Set serial");
    await setSerialAsPartId(page);
  });

  test("Set real serial for one part @manager", async ({ page }) => {
    await indexPage.chooseNumberRowOnTable(1);
    await indexPage.clickDropdownBulkButton("Bulk actions", "Set real serials");
    await tryToSetTwoRealSerialsForOnePart("test_real_serials_1", page);
    await setRealSerial("test_real_serials_1", page);
  });
});
